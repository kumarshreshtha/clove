...
if TYPE_CHECKING:
    from yaad.autodiff import variable
...
class _OperatorsFactory:
    registry = {}

    @classmethod
    def _copy_op(cls, name, op: Operator):
        new_fn = _wrap_func(op.apply)
    ...
    @classmethod
    def register_variable_method(cls, name, op: Operator):
        new_fn = cls._copy_op(name, op)
        sig = inspect.signature(op.forward)
        new_sig = sig.replace(
            parameters=[param for param in sig.parameters.values()
                        if param.name != "input"])
        new_fn.__signature__ = new_sig
        VariableMeta.REGISTERED_OPS[name] = new_fn
