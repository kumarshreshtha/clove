...
from yaad.autodiff import base
...
class ExpOp(base.Operator, fn_name="exp", symbol="e"):
    def forward(self, inp: variable.Variable):
        out = variable.Variable(math.exp(inp.data))
...
class PowOp(base.Operator, fn_name="pow", symbol="**"):
    def forward(self, inp: variable.Variable, other: numbers.Number):
        out = inp.data**other
        self.save_for_backward("inp", inp)
        self.save_for_backward("other", other)
        return out
